# https://github.com/shariqiqbal2810/MAAC/blob/master/utils/misc.py
import os
import torch
...
def sep_clip_grad_norm(parameters, max_norm, norm_type=2):
    ...
    # extract parameter which grad is not None
    ...
    for p in parameters:
        if norm_type == float('inf'):
            p_norm = p.grad_data.abs().max()
        else:
            p_norm = p.grad_data.norm(norm_type)
